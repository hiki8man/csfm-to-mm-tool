...
def _setup_chinese_font(font_size:int=14) -> None:

    dpi_value = get_dpi()
    scaled_size = round(font_size * (dpi_value / 100))

    logger.debug("配置GUI字体")

    from pathlib import Path
    with dpg.font_registry():
        if Path("font\\font.ttf").exists():
            logger.debug(f"字体：font\\font.ttf，字体大小：{scaled_size}")
            with dpg.font("font\\font.ttf", scaled_size, tag="font_base") as font:
                dpg.add_char_remap(0x2764, 0x2665)
